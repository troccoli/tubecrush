...
<div>
    <form wire:submit.prevent="submit">
        ...
        <div class="mb-6 md:w-full md:mr-6">
            ...
            <x-jet-input-error for="title" class="mt-2" dusk="title-error" data-cy="title-error"/>
        </div>
        ...
        <div class="mb-6 md:w-full md:mr-6">
            ...
            <x-jet-input-error for="line" class="" dusk="line-error" data-cy="line-error"/>
        </div>
        ...
        <div class="mb-6">
            ...
            <x-jet-input-error for="content" class="mt-2" dusk="content-error" data-cy="content-error"/>
        </div>

        <!-- Photo -->
        <div class="mb-6">
            <x-upload-photo-button></x-upload-photo-button>
            <x-jet-input-error for="photo" class="mt-2" dusk="photo-error" data-cy="photo-error"/>
            <div class="mt-4">
                @if ($errors->has('photo') || !$photo)
                    <img dusk="photo-image" data-cy="photo-image"
                         src="{{ \Illuminate\Support\Facades\Storage::url($post->getPhoto()) }}"
                         alt="Cover photo">
                @else
        ...
        <div class="mb-6 md:w-full md:mr-6">
            ...
            <x-jet-input-error for="photoCredit" class="mt-2" dusk="photo-credit-error" data-cy="photo-credit-error"/>
        </div>

        <!-- Tags -->
        <div class="mb-6 md:w-full md:mr-6">
            <x-jet-label for="tags" value="{{ __('Tags') }}"/>
            <div wire:ignore class="mt-2" dusk="tags-select" data-cy="tags-select">
                <select id="tags" class="form-input block w-full " name="tags" multiple>
                    @foreach($availableTags as $tag)
                        <option value="{{ $tag['id'] }}"
                                @if(in_array($tag['id'], $tags)) selected @endif>{{ $tag['text'] }}</option>
                    @endforeach
                </select>
            </div>
            <x-jet-input-error for="tags" class="mt-2" dusk="tags-error" data-cy="tags-error"/>
        </div>

        <!-- Buttons -->
        <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 justify-end">
            <x-jet-secondary-button dusk="cancel-button" data-cy="cancel-button" wire:click="cancelEdit"
                                    class="w-full justify-center md:w-auto">{{ __('Cancel') }}</x-jet-secondary-button>
            <x-jet-button dusk="submit-button" data-cy="submit-button" class="w-full justify-center md:w-auto">
                <x-heroicons-s-arrow-path wire:loading wire:target="submit" dusk="submit-loading-icon"
                                          data-cy="submit-loading-icon" class="animate-spin w-4 h-4 mr-1"/>
                {{ __('Update') }}
            </x-jet-button>
        </div>
    </form>
