<div class="md:col-span-3 py-5 md:py-0">
        ...
        <div>
            ...
            <div class="relative border-t border-t-gray-200">
                <div wire:loading.delay class="w-full h-full absolute block top-0 left-0 bg-white opacity-75 z-50"
                     dusk="loading-icon" data-cy="loading-icon">
                    <div class="flex h-full items-center justify-center">
                        <x-loading-icon class=" h-20 w-20 text-gray-900"/>
                    </div>
                </div>
                <div dusk="posts-list" data-cy="posts-list" class="divide-y divide-gray-200">
                    @foreach($posts as $post)
                        <!-- single post row -->
                        <div dusk="post" data-cy="post"
                            ...
                            <div class="w-full md:w-1/2 md:pl-4 text-lg text-gray-900">
                                <span dusk="post-title" data-cy="post-title">{{ $post->getTitle() }}</span>
                                <div class="mt-1 text-sm text-gray-500">
                                    by
                                    <span class="italic" dusk="post-author" data-cy="post-author">
                                        {{ $post->author->getName() }}
                                    </span>
                            ...
                            <div class="flex flex-row items-center mt-4 md:mt-0">
                                @can('update posts')
                                    <a href="{{ route('posts.update', ['postId' => $post->getKey()]) }}"
                                       ...
                                       dusk="edit-post-button" data-cy="edit-post-button">
                                ...
                                @can('delete posts')
                                    <a href="#" wire:click="confirmAction({{ $post->getKey() }}, 'delete')"
                                       ...
                                       dusk="delete-post-button" data-cy="delete-post-button">
                                ...
                                @can('publish posts')
                                    @if($post->isDraft())
                                        <a href="#" wire:click="confirmAction({{ $post->getKey() }}, 'publish')"
                                           ...
                                           dusk="publish-post-button" data-cy="publish-post-button">
                                    ...
                                    @else
                                        <a href="#" wire:click="confirmAction({{ $post->getKey() }}, 'unpublish')"
                                           ...
                                           dusk="unpublish-post-button" data-cy="unpublish-post-button">
    ...
    <x-confirmation-modal wire:model="confirmingDeletingId" id="confirm-delete-post-dialog">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="keepPost" wire:loading.attr="disabled" dusk="cancel-delete-post-button"
                                data-cy="cancel-delete-post-button">
                {{ __("Never mind") }}
            </x-secondary-button>

            <x-danger-button class="ml-2" wire:click="deletePost" wire:loading.attr="disabled"
                             dusk="confirm-delete-post-button" data-cy="confirm-delete-post-button">
                {{ __('Yes please') }}
            </x-danger-button>
        </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmingPublishingId" id="confirm-publish-post-dialog">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="keepPost" wire:loading.attr="disabled"
                                dusk="cancel-publish-post-button" data-cy="cancel-publish-post-button">
                {{ __("Whoops, no thanks") }}
            </x-secondary-button>

            <x-danger-button class="ml-2" wire:click="publishPost" wire:loading.attr="disabled"
                             dusk="confirm-publish-post-button" data-cy="confirm-publish-post-button">
                {{ __('Yep, let\'s go') }}
            </x-danger-button>
        </x-slot>
    </x-confirmation-modal>
    <x-confirmation-modal wire:model="confirmingUnpublishingId" id="confirm-unpublish-post-dialog">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="keepPost" wire:loading.attr="disabled"
                                dusk="cancel-unpublish-post-button" data-cy="cancel-unpublish-post-button">
                {{ __("Naah, leave it") }}
            </x-secondary-button>

            <x-danger-button class="ml-2" wire:click="unpublishPost" wire:loading.attr="disabled"
                             dusk="confirm-unpublish-post-button" data-cy="confirm-unpublish-post-button">
                {{ __('Oh yeah') }}
            </x-danger-button>
        </x-slot>
    </x-confirmation-modal>
